...
- Series expansion of the reciprocal distance
- Simple applications
...
$t$ of the generating function $G(x,t)$

$$
G(x,t) = \frac{1}{\sqrt{1 - 2tx + t^2}} = \sum\limits_{n=0}^\infty
P_n(x) t^{n}.
$$
The coefficient of $t^n$ is a polynomial in $x$ of degree $n$ with $|x| \le 1$.

The entire family $P_{n}(x)$ is **generated** by a single closed-form expression.
Differentiation, algebraic manipulation, or expansion of $G(x,t)$ produces identities involving all $P_n$ simultaneously.
...
The superposition of Legendre polynomials describe the latitude-dependent variations of the potential.

For $r \ll s$, a similar result is obtained by factoring out $1/s$
$$
f(r, \theta) =
\begin{cases}
\dfrac{1}{s} \sum\limits_{n=0}^\infty \left(\dfrac{r}{s}\right)^n P_n(\cos \theta), & r \leq s, \\
\dfrac{1}{r} \sum\limits_{n=0}^\infty \left(\dfrac{s}{r}\right)^{n} P_n(\cos \theta), & r \geq s.
\end{cases}
$$
