...
Here, $\overline \Omega$ is the _closure_ of $\Omega$, which is the smallest closed set containing $\Omega$.
...
On the other hand, the domain $\Omega$ is an open subset of $\mathbb R^{n}$. Open means that every point $x \in \Omega$ has a small ball $B_{\varepsilon}(x)$ that is still contained in $\Omega$. Thus, _the boundary points are not included in $\Omega$_.
