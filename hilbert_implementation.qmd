...
```{python}
def get_wavenumber(n, dt = 1):
    k = fftfreq(n, d = dt)
    return k
```

We construct a sinusoidal signal `S` with a frequency of $f=10$ Hz, a period $T=0.1$ s, and a signal length of $L=2$ s.
It holds
$$
S(t) = \sin(\frac{2 \pi}{T}t) = \sin(2 \pi f t)
$$
The sampling frequency is $200$ Hz, which corresponds to a sampling interval of $\Delta t=0.005$ s and a resulting window length of $n=400$ samples.
...
We recognize that the major Fourier component of the signal is associated with $f=\pm 10$ Hz.
