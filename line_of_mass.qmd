...
```{python}
#| code-fold: true
#| fig-cap: "Potential $V$ of an infinite wire at $y=0$ and $z=5$, $f \\rho_L = 1$."
V = -2 * f * rho_L * log(sqrt((y- yprime)**2 + (z - zprime)**2))
# display(V)
plot(V.subs([(f, 1), (rho_L, 1), (z, 0), (zprime, 5), (yprime, 0)]),
  (y, -5, 5), size=(4, 3),
  xlabel='y', ylabel=r'$V(y)$', ylim=(-4, -3), xlim=(-5, 5), axis_center=(-5, -4))
```

```{python}
#| code-fold: true
#| fig-cap: "Vertical attraction $g_z$ of an infinite wire at $y=0$ and $z=5$, $f \\rho_L = 1$."
plot(g_z.subs([(f, 1), (rho_L, 1), (z, 0), (zprime, 5), (yprime, 0)]), 
  (y, -5, 5), size=(4, 3),
  xlabel='y', ylabel=r'$g_z(y)$', axis_center=(-5, 0), ylim=(0,0.5), xlim=(-5,5))
```
...
$$ Around an elementary prism $\mathrm{d}\Theta\mathrm{d}z$ bounded by the lines $\Theta$ and $\Theta + \mathrm{d}\Theta$, and $z$ and $z+\mathrm{d}z$, consider the line integral $\oint \Theta \mathrm{d}z$. Traversing the circuit in counter-clockwise direction, the resulting integral will be $$
